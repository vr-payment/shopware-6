...
class ActivePaymentRedirectSubscriber implements EventSubscriberInterface
{
    ...
    public function onKernelRequest(RequestEvent $event): void
    {
        ...
        // 'frontend.account.order.page'), a cookie vrpayment_deliberate was set and read here to 
        // prevent this function from wrongly redirecting the user to recreate-cart.
        if ($request->cookies->get('vrpayment_deliberate')) {
            $session->remove('vrpaymentActivePaymentOrderId');
            return;
        }

        // If the user entered checkout, vrpaymentActivePaymentOrderId should’ve been set.
        $orderId = (string) $session->get('vrpaymentActivePaymentOrderId', '');
        if ($orderId === '') {
            return;
        }
