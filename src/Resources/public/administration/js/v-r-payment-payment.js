...
{% endblock %}
`;var{Component:re,Context:C}=Shopware,oe=Shopware.Data.Criteria,se="handler_vrpaymentpayment_vrpaymentpaymenthandler";re.override("sw-order-detail",{template:I,data(){return{isVRPaymentPayment:!1}},computed:{isEditable(){return!this.isVRPaymentPayment||this.$route.name!=="vrpayment.order.detail"},showTabs(){return!0}},watch:{orderId:{deep:!0,handler(){if(!this.orderId){this.setIsVRPaymentPayment(null);return}let e=this.repositoryFactory.create("order"),t=new oe(1,1);t.addAssociation("transactions"),e.get(this.orderId,C.api,t).then(a=>{if(a.amountTotal<=0||a.transactions.length<=0||!a.transactions[0].paymentMethodId){this.setIsVRPaymentPayment(null);return}let n=a.transactions[0].paymentMethodId;n!=null&&this.setIsVRPaymentPayment(n)})},immediate:!0}},methods:{setIsVRPaymentPayment(e){if(!e)return;this.repositoryFactory.create("payment_method").get(e,C.api).then(a=>{this.isVRPaymentPayment=a.formattedHandlerIdentifier===se})}}});var E=`{% block vrpayment_order_action_completion %}
...
{% endblock %}
`;var{Component:ce,Mixin:de,Filter:me,Utils:S}=Shopware;ce.register("vrpayment-order-action-completion",{template:E,inject:["VRPaymentTransactionCompletionService"],mixins:[de.getByName("notification")],props:{transactionData:{type:Object,required:!0}},data(){return{isLoading:!0,isCompletion:!1}},computed:{dateFilter(){return me.getByName("date")}},created(){this.createdComponent()},methods:{createdComponent(){this.isLoading=!1},completion(){this.isCompletion&&(this.isLoading=!0,this.VRPaymentTransactionCompletionService.createTransactionCompletion(this.transactionData.transactions[0].metaData.salesChannelId,this.transactionData.transactions[0].id).then(()=>{this.createNotificationSuccess({title:this.$tc("vrpayment-order.captureAction.successTitle"),message:this.$tc("vrpayment-order.captureAction.successMessage")}),this.isLoading=!1,this.$emit("modal-close"),this.$nextTick(()=>{this.$router.replace(`${this.$route.path}?hash=${S.createId()}`)})}).catch(e=>{try{this.createNotificationError({title:e.response.data.errors[0].title,message:e.response.data.errors[0].detail,autoClose:!1})}catch{this.createNotificationError({title:e.title,message:e.message,autoClose:!1})}finally{this.isLoading=!1,this.$emit("modal-close"),this.$nextTick(()=>{this.$router.replace(`${this.$route.path}?hash=${S.createId()}`)})}}))}}});var w=`{% block vrpayment_order_action_refund %}
<sw-modal variant="small"
...
{% endblock %}
`;var{Component:pe,Mixin:he,Filter:fe,Utils:A}=Shopware;pe.register("vrpayment-order-action-refund",{template:w,inject:["VRPaymentRefundService"],mixins:[he.getByName("notification")],props:{transactionData:{type:Object,required:!0},orderId:{type:String,required:!0}},data(){return{refundQuantity:0,isLoading:!0,currentLineItem:""}},computed:{dateFilter(){return fe.getByName("date")}},created(){this.createdComponent()},methods:{createdComponent(){this.isLoading=!1,this.refundQuantity=1},refund(){this.isLoading=!0,this.VRPaymentRefundService.createRefund(this.transactionData.transactions[0].metaData.salesChannelId,this.transactionData.transactions[0].id,this.refundQuantity,this.$parent.$parent.currentLineItem).then(()=>{this.createNotificationSuccess({title:this.$tc("vrpayment-order.refundAction.successTitle"),message:this.$tc("vrpayment-order.refundAction.successMessage")}),this.isLoading=!1,this.$emit("modal-close"),this.$nextTick(()=>{this.$router.replace(`${this.$route.path}?hash=${A.createId()}`)})}).catch(e=>{try{var t=e?.response?.data?.errors?.[0]?.title??this.$tc("vrpayment-order.refundAction.refundCreateError.errorTitle"),a;switch(e.response.data){case"refundQuantityZero":a=this.$tc("vrpayment-order.refundAction.refundCreateError.messageRefundQuantityIsZero");break;case"refundExceedsQuantity":a=this.$tc("vrpayment-order.refundAction.refundCreateError.messageRefundQuantityExceedsAvailableBalance");break;case"methodDoesNotSupportRefund":a=this.$tc("vrpayment-order.refundAction.refundCreateError.messagePaymentMethodDoesNotSupportRefund");break;default:a=e.response.data.errors[0].detail}this.createNotificationError({title:t,message:a,autoClose:!1})}catch{this.createNotificationError({title:e.title,message:e.message,autoClose:!1})}finally{this.isLoading=!1,this.$emit("modal-close"),this.$nextTick(()=>{this.$router.replace(`${this.$route.path}?hash=${A.createId()}`)})}})}}});var T=`{% block vrpayment_order_action_refund_partial %}
<sw-modal variant="small"
		  :title="$tc(\`vrpayment-order.modal.title.refund\`)"
		  @modal-close="$emit('modal-close')">
...
{% endblock %}
`;var{Component:Ie,Mixin:Ce,Filter:Ee,Utils:N}=Shopware;Ie.register("vrpayment-order-action-refund-by-amount",{template:D,inject:["VRPaymentRefundService"],mixins:[Ce.getByName("notification")],props:{transactionData:{type:Object,required:!0},orderId:{type:String,required:!0}},data(){return{isLoading:!0,currency:this.transactionData.transactions[0].currency,refundAmount:0,refundableAmount:0}},computed:{dateFilter(){return Ee.getByName("date")}},created(){this.createdComponent()},methods:{createdComponent(){this.isLoading=!1,this.currency=this.transactionData.transactions[0].currency,this.refundAmount=Number(this.transactionData.transactions[0].amountIncludingTax),this.refundableAmount=Number(this.transactionData.transactions[0].amountIncludingTax)},refundByAmount(){this.isLoading=!0,this.VRPaymentRefundService.createRefundByAmount(this.transactionData.transactions[0].metaData.salesChannelId,this.transactionData.transactions[0].id,this.refundAmount).then(()=>{this.createNotificationSuccess({title:this.$tc("vrpayment-order.refundAction.successTitle"),message:this.$tc("vrpayment-order.refundAction.successMessage")}),this.isLoading=!1,this.$emit("modal-close"),this.$nextTick(()=>{this.$router.replace(`${this.$route.path}?hash=${N.createId()}`)})}).catch(e=>{try{var t=e?.response?.data?.errors?.[0]?.title??this.$tc("vrpayment-order.refundAction.refundCreateError.errorTitle"),a;switch(e.response.data){case"refundAmountZero":a=this.$tc("vrpayment-order.refundAction.refundCreateError.messageRefundAmountIsZero");break;case"refundExceedsAmount":a=this.$tc("vrpayment-order.refundAction.refundCreateError.messageRefundAmountExceedsAvailableBalance");break;case"methodDoesNotSupportRefund":a=this.$tc("vrpayment-order.refundAction.refundCreateError.messagePaymentMethodDoesNotSupportRefund");break;default:a=e.response.data.errors[0].detail}this.createNotificationError({title:t,message:a,autoClose:!1})}catch{this.createNotificationError({title:e.title,message:e.message,autoClose:!1})}finally{this.isLoading=!1,this.$emit("modal-close"),this.$nextTick(()=>{this.$router.replace(`${this.$route.path}?hash=${N.createId()}`)})}})}}});var R=`{% block vrpayment_order_action_void %}
<sw-modal variant="small"
...
{% endblock %}
`;var{Component:et,Mixin:tt}=Shopware;et.register("sw-vrpayment-credentials",{template:z,name:"VRPaymentCredentials",inject:["acl"],mixins:[tt.getByName("notification")],props:{actualConfigData:{type:Object,required:!0},allConfigs:{type:Object,required:!0},selectedSalesChannelId:{type:[String,null],required:!1,default:null},spaceIdFilled:{type:Boolean,required:!0},spaceIdErrorState:{required:!0},userIdFilled:{type:Boolean,required:!0},userIdErrorState:{required:!0},applicationKeyFilled:{type:Boolean,required:!0},applicationKeyErrorState:{required:!0},isLoading:{type:Boolean,required:!0},isTesting:{type:Boolean,required:!1}},data(){return{...l}},computed:{currentConfig(){return this.selectedSalesChannelId&&this.allConfigs[this.selectedSalesChannelId]?this.allConfigs[this.selectedSalesChannelId]:this.allConfigs.null||{}}},methods:{checkTextFieldInheritance(e){return!e||e.length<=0},checkNumberFieldInheritance(e){return e==null||e===""},checkBoolFieldInheritance(e){return typeof e!="boolean"},emitCheckApiConnectionEvent(){let e={spaceId:this.currentConfig[l.CONFIG_SPACE_ID],userId:this.currentConfig[l.CONFIG_USER_ID],applicationKey:this.currentConfig[l.CONFIG_APPLICATION_KEY]};this.$emit("check-api-connection-event",e)},getInheritedValue(e){return this.allConfigs.null?.[e]??null}}});var G=`{% block vrpayment_settings_content_card_channel_config_options %}
			...
			<sw-container>
				...
				{% block vrpayment_settings_content_card_channel_config_credentials_card_container_settings %}
					<div v-if="actualConfigData" class="vrpayment-settings-options-fields">
						...
						{% block vrpayment_settings_content_card_channel_config_credentials_card_container_settings_order_close_enabled %}
							<sw-inherit-wrapper
								...
								<template #content="props">
									<sw-switch-field
											...
											bordered
											:mapInheritance="props"
											:label="$tc('vrpayment-settings.settingForm.options.orderCloseEnabled.label')"
											:helpText="$tc('vrpayment-settings.settingForm.options.orderCloseEnabled.tooltipText')"
											:disabled="props.isInherited"
											:value="props.currentValue"
											@update:value="props.updateCurrentValue">
									</sw-switch-field>
						...
						{% block vrpayment_settings_content_card_channel_config_credentials_card_container_settings_product_custom_fields_allow_list %}
							<sw-inherit-wrapper
								...
								<template #content="props">
									<sw-multi-select
											...
											:mapInheritance="props"
											:label="$tc('vrpayment-settings.settingForm.options.productCustomFieldsAllowList.label')"
											:helpText="$tc('vrpayment-settings.settingForm.options.productCustomFieldsAllowList.tooltipText')"
											:disabled="props.isInherited"
											:value="props.currentValue || []"
											@update:value="props.updateCurrentValue">
									</sw-multi-select>
...
{% endblock %}
`;var{Component:nt,Mixin:it,Context:rt}=Shopware,f=Shopware.Data.Criteria;nt.register("sw-vrpayment-options",{template:G,name:"VRPaymentOptions",inject:["repositoryFactory"],mixins:[it.getByName("notification")],props:{actualConfigData:{type:Object,required:!0},allConfigs:{type:Object,required:!0},selectedSalesChannelId:{required:!0},isLoading:{type:Boolean,required:!0}},data(){return{...l,productCustomFields:[]}},created(){this.loadProductCustomFields()},computed:{customFieldRepository(){return this.repositoryFactory.create("custom_field")},productCustomFieldOptions(){let e=Shopware.Store&&Shopware.Store.get?Shopware.Store.get("session"):null,t=e&&e.currentLocale||"en-GB";return this.productCustomFields.map(a=>{let n=a.config&&a.config.label?a.config.label[t]||a.config.label["en-GB"]:null;return{value:a.name,label:n&&n!==a.name?`${n} (${a.name})`:a.name}})},integrationOptions(){return[{id:"payment_page",name:this.$tc("vrpayment-settings.settingForm.options.integration.options.payment_page")},{id:"iframe",name:this.$tc("vrpayment-settings.settingForm.options.integration.options.iframe")}]}},methods:{loadProductCustomFields(){let e=new f(1,500);e.addFilter(f.equals("customFieldSet.relations.entityName","product")),e.addSorting(f.sort("name","ASC")),this.customFieldRepository.search(e,rt.api).then(t=>{this.productCustomFields=t})},checkMultiSelectFieldInheritance(e){return!Array.isArray(e)||e.length<=0},checkTextFieldInheritance(e){return typeof e!="string"?!0:e.length<=0},checkNumberFieldInheritance(e){return typeof e!="number"?!0:e.length<=0},checkBoolFieldInheritance(e){return typeof e!="boolean"}}});var q=`{% block vrpayment_settings_icon %}
...
{% endblock %}
`;var{Component:st}=Shopware;st.register("sw-vrpayment-settings-icon",{template:q});var U=`<mt-card class="mt-card"
...
`;var{Component:ct,Mixin:dt}=Shopware;ct.register("sw-vrpayment-storefront-options",{template:U,name:"VRPaymentStorefrontOptions",mixins:[dt.getByName("notification")],props:{actualConfigData:{type:Object,required:!0},allConfigs:{type:Object,required:!0},selectedSalesChannelId:{required:!0},isLoading:{type:Boolean,required:!0}},data(){return{...l}},methods:{checkTextFieldInheritance(e){return typeof e!="string"?!0:e.length<=0},checkNumberFieldInheritance(e){return typeof e!="number"?!0:e.length<=0},checkBoolFieldInheritance(e){return typeof e!="boolean"}}});var H=`<mt-card class="mt-card"
